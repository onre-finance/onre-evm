...
contract OnReMathTest is Test {
    ...
    uint8 private constant MANAGED_TOKEN_DECIMALS = 9;
    ...
    uint256 private constant MAX_OPERATIONAL_MANAGED_TOKEN_SUPPLY = 18_000_000_000e9;
    ...
    function test_OperationalEighteenBillionValuesDoNotOverflow() public view {
        assertEq(
            math.calculateTokenOutAmount(
                MAX_OPERATIONAL_USDC_AMOUNT, PRICE_SCALE, USDC_DECIMALS, MANAGED_TOKEN_DECIMALS
            ),
            MAX_OPERATIONAL_MANAGED_TOKEN_SUPPLY
        );
        assertEq(
            math.calculateRedemptionAssetOutAmount(
                MAX_OPERATIONAL_MANAGED_TOKEN_SUPPLY, PRICE_SCALE, MANAGED_TOKEN_DECIMALS, USDC_DECIMALS
            ),
            MAX_OPERATIONAL_USDC_AMOUNT
        );
        assertEq(
            math.calculateTvl(MAX_OPERATIONAL_MANAGED_TOKEN_SUPPLY, PRICE_SCALE, PRICE_SCALE),
            MAX_OPERATIONAL_MANAGED_TOKEN_SUPPLY
        );
        assertEq(
    ...
    function testFuzz_OfferQuoteSupportsEighteenBillionUsdcInputs(uint96 rawAssetAmount, uint64 rawPrice) public view {
        ...
        uint256 tokenOut = math.calculateTokenOutAmount(assetAmount, price, USDC_DECIMALS, MANAGED_TOKEN_DECIMALS);
    ...
    function testFuzz_OfferQuoteSupportsEighteenBillionDaiInputs(uint96 rawAssetAmount, uint64 rawPrice) public view {
        ...
        uint256 tokenOut = math.calculateTokenOutAmount(assetAmount, price, DAI_DECIMALS, MANAGED_TOKEN_DECIMALS);
        uint256 expectedTokenOut = Math.mulDiv(assetAmount, PRICE_SCALE, price * 1e9);

        assertEq(tokenOut, expectedTokenOut);
    }

    function testFuzz_RedemptionQuoteSupportsEighteenBillionUsdcOutputs(uint96 rawManagedAmount, uint64 rawPrice)
        public
        view
    {
        uint256 managedAmount = bound(uint256(rawManagedAmount), 1, MAX_OPERATIONAL_MANAGED_TOKEN_SUPPLY);
        uint256 price = bound(uint256(rawPrice), 1, MAX_OPERATIONAL_PRICE);

        uint256 assetOut =
            math.calculateRedemptionAssetOutAmount(managedAmount, price, MANAGED_TOKEN_DECIMALS, USDC_DECIMALS);
        uint256 expectedAssetOut = Math.mulDiv(managedAmount, price, 1e12);

        assertEq(assetOut, expectedAssetOut);
    }

    function testFuzz_RedemptionQuoteSupportsEighteenBillionDaiOutputs(uint96 rawManagedAmount, uint64 rawPrice)
        public
        view
    {
        uint256 managedAmount = bound(uint256(rawManagedAmount), 1, MAX_OPERATIONAL_MANAGED_TOKEN_SUPPLY);
        ...
        uint256 assetOut =
            math.calculateRedemptionAssetOutAmount(managedAmount, price, MANAGED_TOKEN_DECIMALS, DAI_DECIMALS);
        uint256 expectedAssetOut = Math.mulDiv(managedAmount, price * 1e9, PRICE_SCALE);

        assertEq(assetOut, expectedAssetOut);
    }

    function testFuzz_TvlUsesFullPrecisionForOperationalSupply(uint96 rawSupply, uint64 rawNav) public view {
        uint256 circulatingSupply = bound(uint256(rawSupply), 0, MAX_OPERATIONAL_MANAGED_TOKEN_SUPPLY);
    ...
    function testFuzz_RedemptionVaultRefillSupportsEighteenBillionTvl(
        ...
        uint256 tvl = bound(uint256(rawTvl), 1, MAX_OPERATIONAL_MANAGED_TOKEN_SUPPLY);
        ...
        uint256 refill = math.calculateRedemptionVaultRefillAmount(
            tvl, vaultTargetBps, MAX_BASIS_POINTS, USDC_DECIMALS, MANAGED_TOKEN_DECIMALS, currentVault, assetNetAmount
        );
    ...
    function test_ExtremeInputsRevertInsteadOfWrapping() public {
        ...
        vm.expectRevert();
        math.calculateRedemptionAssetOutAmount(
            type(uint256).max, type(uint256).max, MANAGED_TOKEN_DECIMALS, DAI_DECIMALS
        );
...
contract OnReMathFuzzHarness {
    ...
    function calculateRedemptionAssetOutAmount(
        ...
        return OnReMath._calculateRedemptionAssetOutAmount(
            managedTokenNetAmount, price, managedTokenDecimals, assetDecimals
        );
    }
