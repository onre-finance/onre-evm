...
import {DiamondOwnershipFacet} from "../src/diamond/facets/DiamondOwnershipFacet.sol";
...
contract DeployOnReDiamond is Script {
    function run() external returns (OnReDiamond diamond) {
        ...
        address diamondOwner = vm.envOr("ONRE_DIAMOND_OWNER", params.admin);

        vm.startBroadcast(deployerKey);
        diamond = _deploy(diamondOwner, params);
        vm.stopBroadcast();
    }

    function _deploy(OnReTypes.InitializeParams memory params) internal returns (OnReDiamond diamond) {
        return _deploy(params.admin, params);
    }

    function _deploy(address diamondOwner, OnReTypes.InitializeParams memory params)
        internal
        returns (OnReDiamond diamond)
    {
        IDiamondCut.FacetCut[] memory cut = new IDiamondCut.FacetCut[](12);
        ...
        cut[2] = _facetCut(address(new DiamondOwnershipFacet()), LibOnReSelectors.diamondOwnership());
        cut[3] = _facetCut(address(new OnReAccessControlFacet()), LibOnReSelectors.accessControl());
        cut[4] = _facetCut(address(new OnReConfigFacet()), LibOnReSelectors.config());
        cut[5] = _facetCut(address(new OnRePricerFacet()), LibOnReSelectors.pricer());
        cut[6] = _facetCut(address(new OnReQuoterFacet()), LibOnReSelectors.quoter());
        cut[7] = _facetCut(address(new OnReOfferFacet()), LibOnReSelectors.offer());
        cut[8] = _facetCut(address(new OnReFulfillmentFacet()), LibOnReSelectors.fulfillment());
        cut[9] = _facetCut(address(new OnReViewFacet()), LibOnReSelectors.viewFunctions());
        cut[10] = _facetCut(address(new OnReConfigurableVaultFacet()), LibOnReSelectors.configurableVault());
        cut[11] = _facetCut(address(new OnReMarketStatsFacet()), LibOnReSelectors.marketStats());

        OnReDiamondInit init = new OnReDiamondInit();
        diamond = new OnReDiamond(diamondOwner, cut, address(init), abi.encodeCall(OnReDiamondInit.init, (params)));
    }
